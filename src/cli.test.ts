...
describe('cli', () => {
  it('preview --fixture rich-real renders candidate summary section', async () => {
    const { stdout, deps } = createDeps();
    await runCli(['preview', '--fixture', 'rich-real'], deps).catch(() => undefined);
    expect(stdout.join('\n')).toContain('═══ 📊 候选库总览 (共 6) ═══');
  });

  it('preview --fixture rich-real --raw outputs assembled json', async () => {
    const { stdout, deps } = createDeps();
    await runCli(['preview', '--fixture', 'rich-real', '--raw'], deps).catch(() => undefined);
    expect(stdout.join('\n')).toContain('"total_candidates": 6');
  });
  ...
  it('notify calls sender and archive', async () => {
    ...
    writeFileSync(eventPath, JSON.stringify({
      ...
      runtime: 'openclaw',
      workspace: '/home/mcdowell/.openclaw/workspace',
      ...
      candidates_summary: { pending: 0, reviewing: 0, shadow: 0, graduated: 0 },
      errors: [],
    }));
